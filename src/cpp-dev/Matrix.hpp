...
public:
	...
	friend ifstream &operator>>(ifstream &is, Matrix &obj);
	...
	void zeromean( bool flag = true);//默认参数为true计算列
	void normalize( bool flag = true);//默认参数为true计算列
...
void Matrix::zeromean( bool flag)
{
	if (flag == true) //计算列均值
	{
...
void Matrix::normalize( bool flag)
{
	if (flag == true) //计算列均值
	{
		double *mean = new double[col];
...
ifstream &operator>>(ifstream &is, Matrix &obj)
{
	for (unsigned i = 0; i<obj.size; i++)
	{
		is >> obj.pmm[i];
...
ostream &operator<<(ostream &out, Matrix &obj)
{
	out << "\n";
	//out.setf(ios::fixed);
	for (unsigned i = 0; i < obj.row; i++) //打印矩阵        
	{
		out << "\t";
		for (unsigned j = 0; j < obj.col; j++)
		{
			out /*<< setprecision(4)*/ << (obj[i][j]) << "\t";
		}
		out << endl;
	}
	return out;
}
ofstream &operator<<(ofstream &out, Matrix &obj)//打印矩阵到文件  
{
